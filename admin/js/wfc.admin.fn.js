     ...
     */
    $('.type-campaign, .type-page, .type-homeboxes, .type-news, .type-subpagebanner, .type-post').click(function (e) {
        if (e.target.nodeName == 'A' || e.target.nodeName == 'INPUT') {
            return;
        } // best way to exlude any native anchor tags ie quick edit, delete
        //console.log($(this));
        url = $('.row-actions .edit a', $(this)).attr('href');
        //console.log(url);
        window.location.href = url;
    });
    $('.type-campaign, .type-page, .type-homeboxes, .type-news, .type-subpagebanner, .type-post').hover(
        function () {
