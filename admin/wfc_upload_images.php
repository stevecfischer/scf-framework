<?php
   ...
   <?php for( $i = 0; $i < count( $images ); $i++ ) : ?>
                ...
                <tr valign="top" id="row_<?php echo $i + 1; ?>">
                    ...
                    <td>
                        <input type="text" name="image_urls[]" id="image_<?php echo
                            $i + 1; ?>" size="70" value="<?php echo  $images[$i]; ?>"/> <input id="image_<?php echo$i +
                        1; ?>" onclick="open_media_uploader(this.id)" type="button" value="Upload Image" class="rttheme_upload_button button"/>
                        <input onclick="remove_field(this.id)" id="row_<?php echo
                            $i + 1; ?>" type="button" value="Remove picture"/><br/> <label for="caption_<?php echo
                        $i + 1; ?>">Caption : </label> <input type="text" name="image_captions[]" id="caption_<?php echo
                        $i + 1; ?>" size="59" value="<?php echo  $captions[$i]; ?>"/>
                    </td>
                </tr>
      ...
      <tr valign="top" id="row_1">
          ...
          <td>
              <a onclick="remove_field(this.id)" id="row_1" href="#">Remove picture</a>
          </td>
          </td>
      </tr>
   ...
   <br/>&nbsp;&nbsp;&nbsp;
    ...
    function scf_load_scripts(){
        wp_enqueue_script(
            'jquery-scf-ideagallery', WFC_JS_URI.'/jquery.ideagallery.1.1.js', array(
                                                                                    'jquery',
                                                                                    'jquery-scf-lightbox'
                                                                               ) );
