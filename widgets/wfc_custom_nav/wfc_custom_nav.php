...
class WFC_Custom_Nav_Widget extends WP_Widget {
   ...
   function WFC_Custom_Nav_Widget(){
      $widget_ops = array('classname' => 'wfc_custom_nav', 'description' => __("WFC Custom Sidebar Navigation") );
   ...
   function widget($argsw, $instance){
      ...
      echo '<ul>';
      // show parent title
      echo '<li class="previous"><a class="return" href="' . get_permalink($post->post_parent) . '">'.get_the_title($post->post_parent).'</a></li>';
      $children = wp_list_pages(array( 'title_li' => '', 'echo' => 1, 'depth' => $thedepth, 'child_of' => $top_page, 'exclude' => $exclude_list, 'walker' => new Description_Walker ));
      echo '</ul>';
   }

   function update($new_instance, $old_instance){
      $instance = $old_instance;
      $instance['side'] = ( in_array( $new_instance['side'], array( 'left', 'right' ) ) ) ? $new_instance['side'] : 'left';
      return $instance;
   }
...
class Description_Walker extends Walker_page {
   function start_el(&$output, $page, $depth, $args, $current_page) {
      ...
      if( isset($short_cut) && !empty($short_cut) ) {
         if( isset($short_new_tab) && !empty($short_new_tab) ) {
            $output .= $indent . '<li class="' . $css_class . '"><a target="_blank" href="' . $short_cut . '">' . $link_before . apply_filters( 'the_title', $page->post_title, $page->ID ) . $link_after . '</a>';
         }else{
            $output .= $indent . '<li class="' . $css_class . '"><a href="' . $short_cut . '">' . $link_before . apply_filters( 'the_title', $page->post_title, $page->ID ) . $link_after . '</a>';
         }
      }else{
