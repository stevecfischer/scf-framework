<?php
    ...
    function wfc_callsLeft(){
        ...
        if( $limit->rate->remaining == 0 ){
            echo '<span style="display:block;color:red;font-size:25px;padding-top:25px;">0 call remaining, reset at '.date( 'h:i:s A', $limit->rate->reset ).
                '</span><br /><br /><br />';
            return 0;
        } else{
    ...
    function wfc_manage_update(){
        ...
        if( isset($_GET['force_update']) && $_GET['force_update'] == true && !empty($_POST["update_url"]) ){
            ...
            if( isset($_GET['check_diffs']) && $_GET['check_diffs'] == true && wfc_callsLeft() > 0 ){
                echo wfc_check_diffs();
            } else{
                if( isset($_GET['update']) && $_GET['update'] == $token && wfc_callsLeft() > 0 ){
                    echo wfc_doUpdate();
                } else{
                    if( wfc_callsLeft() > 0 ){
    ...
    function broken_github(){
        echo 'GitHub is broken ? Update from there : <form method="POST" action ="'.$_SERVER['PHP_SELF'].
            '?page=wfc_theme_customizer.php&force_update=true"><input type="text" value="" name="update_url" /><input type="submit" value="Update" onclick="return confirm(\'Did you do a backup before ? Backup Manager can help you with that !\');" /></form>';
    }
                        ...
                        foreach( $old_files as $f => $sha ){
                            if( !in_array( $f, unserialize( IGNORE ) ) ){
                                if( array_key_exists( $f, $current_files ) ){
                                    ...
                                    $bad++;
                                    $missing .=
                                        '<span style="color:#FF6600;font-weight:bold;">File : <strong style="color:black;">'.$f.'</strong> is missing on local.</span><br />';
                                }
                            }
                        }
    ...
    function wfc_print_api_limit(){
        ...
        $context = stream_context_create( $options );
        $limit   = json_decode( @file_get_contents( 'https://api.github.com/rate_limit', false, $context ) );
        echo '<div style="width:100%;text-align:center;"><span style="color:blue;font-size:15px;">'.$limit->rate->remaining.' calls remaining, reset at '.
            date( 'h:i:s A', $limit->rate->reset ).'</span></div>';
    }
    ...
    function wfc_DisplayMonitor( $monitor ){
        echo '<div style="width:100%;text-align:center;">Execution time : '.Monitor::HumanTime( $monitor->GetElapsedTime() ).' - Max memory allocated : '.
            Monitor::HumanSize( $monitor->GetMemoryPeak() ).'</div>';
    }
        ...
        foreach( $ffs as $ff ){
            if( is_array( $exclude ) and !in_array( $ff, $exclude ) ){
                if( $ff != '.' && $ff != '..' ){
                    ...
                    if( is_dir( $dir.'/'.$ff ) ){
                        $files = listFolderFilesArr( $dir.'/'.$ff, $exclude, $strip, $files );
                    }
                }
            }
        }
