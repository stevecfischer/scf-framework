...
=======
    ...
    function wfc_callsLeft(){
        ...
        $limit = json_decode( @file_get_contents( 'https://api.github.com/rate_limit' ) );
        if( $limit->rate->remaining == 0 ){
            echo '<span style="color:red;font-size:25px;margin-top:25px;">0 call remaining, reset at '.
                date( 'h:i:s A', $limit->rate->reset ).'</span><br /><br /><br />';
        } else{
            return $limit->rate->remaining;
        }
    }
    ...
    function wfc_generateToken(){
        $today = time();
        return $token = sha1(
            mktime( date( 'H', $today ), 0, 0, date( 'n', $today ), date( 'j', $today ), date( 'Y', $today ) ).
            $_SERVER['PHP_SELF'] );
>>>>>>> dd1485caa6d61f39c8295a2d3fc44b3a23e5a0e1
    }
    ...
    function wfc_check_update(){
        ...
        <form method="POST" action ="'.$_SERVER['PHP_SELF'].
                            '?page=wfc_theme_customizer.php&check_diffs=true"><input type="submit" value="Check diffs" /></form>';
                    }
                }
            }
        }
            ...
            foreach( $tags as $tag ){
                if( $tag->name == $loc ){
                    ...
                    if( $folder_name != '' ){
                        ...
                        $current_files    =
                            listFolderFilesArr( $path_to_current, array(), $current_nb_carac, $current_files );
                        ...
                        */
                        ...
                        foreach( $old_files as $f => $sha ){
                            if( !in_array( $f, unserialize( IGNORE ) ) ){
                                if( array_key_exists( $f, $current_files ) ){
                                    ...
                                    $bad++;
                                    $missing .=
                                        '<span style="color:#FF6600;font-weight:bold;">File : <strong style="color:black;">'.
                                        $f.'</strong> is missing on local.</span><br />';
                                }
                            }
                        }
                        foreach( $current_files as $f => $sha ){
                            if( !in_array( $f, unserialize( IGNORE ) ) ){
                                ...
                                $missing .=
                                    '<span style="color:#FF6600;font-weight:bold;">File : <strong style="color:black;">'.
                                    $f.'</strong> is missing on GitHub.</span><br />';
                            }
                        }
                        ...
                        if( $do_update ){
                            ...
                            Click on the button to proceed to the update : <form method="POST" action ="'.
                                $_SERVER['PHP_SELF'].'?update='.
                            ...
                            <form method="POST" action ="'.$_SERVER['PHP_SELF'].
                                '?page=wfc_theme_customizer.php&update='.
                                $token.'"><input type="submit" value="Update" /></form>';
                        }
    ...
    function wfc_print_api_limit(){
        $limit = json_decode( @file_get_contents( 'https://api.github.com/rate_limit' ) );
        echo '<div style="width:100%;text-align:center;"><span style="color:blue;font-size:15px;">'.
            $limit->rate->remaining.' calls remaining, reset at '.date( 'h:i:s A', $limit->rate->reset ).
            '</span></div>';
    }
    ...
    function wfc_DisplayMonitor( $monitor ){
        echo '<div style="width:100%;text-align:center;">Execution time : '.
            Monitor::HumanTime( $monitor->GetElapsedTime() ).' - Max memory allocated : '.
